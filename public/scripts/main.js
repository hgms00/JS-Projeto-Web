...
function buy()
{
    for(i=0;i<qt_carrinho;i++)
        ...
        alert("Foi comprado:" +     
                "\nNOME: " + myCarrinho[i].nome + 
                "\nPREÇO POR UNIDADE: " + myCarrinho[i].preco+
                "\nQUANTIDADE: " + myCarrinho[i].quantidade);
            
            
            var indice = findProduct(myCarrinho[i].ID);
            produto[indice].quantidade -= myCarrinho[i].quantidade;
            myCarrinho[i]=null;
        
        }
    
    alert("TODOS OS ITENS DO CARRINHO FORAM COMPRADOS COM SUCESSO!");
    resetFormulario();
    panelChanger("RESETAR");
}
...
function panelChanger(panel){
    ...
    btn[8] = document.getElementById('COMPRAR').classList;
